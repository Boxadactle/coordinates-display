...
public class HudOverlay extends DrawableHelper {
    ...
    public void renderOverlay(MatrixStack matrices, Vec3d pos, ChunkPos chunkPos, float cameraYaw, @Nullable RegistryEntry<Biome> biome, int x, int y) throws NullPointerException {
        ...
        Text xText = new TranslatableText("hud.coordinatesdisplay.x", CoordinatesDisplay.DataColorPrefix + (decimalFormat.format(pos.getX())));
        Text yText = new TranslatableText("hud.coordinatesdisplay.y", CoordinatesDisplay.DataColorPrefix + (decimalFormat.format(pos.getY())));
        Text zText = new TranslatableText("hud.coordinatesdisplay.z", CoordinatesDisplay.DataColorPrefix + (decimalFormat.format(pos.getZ())));

        Text chunkX = new TranslatableText("hud.coordinatesdisplay.chunk.x", CoordinatesDisplay.DataColorPrefix + chunkPos.x);
        Text chunkZ = new TranslatableText("hud.coordinatesdisplay.chunk.z", CoordinatesDisplay.DataColorPrefix + chunkPos.z);

        float yaw = MathHelper.wrapDegrees(cameraYaw);
        String direction = new TranslatableText("hud.coordinatesdisplay.direction." + ModUtils.getDirectionFromYaw(yaw)).getString();
        Text directionText = this.config.renderDirection ? new TranslatableText("hud.coordinatesdisplay.direction", direction, String.format("%s(%s)", CoordinatesDisplay.DataColorPrefix, decimalFormat.format(yaw))) : new LiteralText("");

        Text biomeText;
        if (biome != null && this.client.world != null) {
            biomeText = this.config.renderBiome ? new TranslatableText("hud.coordinatesdisplay.biome", CoordinatesDisplay.DataColorPrefix + ModUtils.getBiomeId(ModUtils.getBiomeString(biome))) : new LiteralText("");
        }
        else
            biomeText = new TranslatableText("hud.coordinatesdisplay.biome", CoordinatesDisplay.DataColorPrefix + "Plains");
