...
public class HudOverlay extends DrawableHelper {
    ...
    public void renderOverlay(MatrixStack matrices, Vec3d pos, ChunkPos chunkPos, float cameraYaw, @Nullable Biome biome, int x, int y) throws NullPointerException {
        ...
        if (biome != null && this.client.world != null) {
            ...
            biomeText = this.config.renderBiome ? new TranslatableText("hud.coordinatesdisplay.biome", CoordinatesDisplay.DataColorPrefix + ModUtils.getBiomeId(registry.getId(biome).toString())) : new LiteralText("");
        }
        else
