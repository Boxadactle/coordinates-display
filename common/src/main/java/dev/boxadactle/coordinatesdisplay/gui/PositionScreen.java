...
public class PositionScreen extends BOptionScreen implements HudHelper {
    ...
    public PositionScreen(Screen parent) {
        super(parent, Component.translatable("screen.coordinatesdispaly.hudposition"));
    ...
    protected void init() {
        ...
        if (!config().renderMode.getMetadata().allowMove()) {
            ...
            Prompts.alert(lastScreen, Component.translatable("message.coordinatesdisplay.movehud.error", config().renderMode.getComponent()));
    ...
    protected void initFooter(LinearLayout layout) {
        layout.addChild(createSaveButton(b -> this.onClose()));
    }
